// File: RuntimeLODApplicator4.cs
// Stage 3 — Applies 4-value predicted threshold vectors to all LODGroup components.
// Each LOD transition (LOD0→1, LOD1→2, LOD2→3, LOD3→Culled) gets its own threshold.
...
public class RuntimeLODApplicator4 : MonoBehaviour
{
    [Header("References")]
    public LODThresholdPredictor4 predictor;
    ...
    void Start()
    {
        ...
        if (predictor == null)
            UnityEngine.Debug.LogError("[RuntimeLODApplicator4] No LODThresholdPredictor4 found.");

        _lodGroups = Object.FindObjectsByType<LODGroup>(FindObjectsSortMode.None);
        UnityEngine.Debug.Log($"[RuntimeLODApplicator4] Found {_lodGroups.Length} LODGroups.");

        CacheLODs();

        // disable stack trace extraction for remaining log calls
    ...
    void LateUpdate()
    {
        ...
        // skip if prediction unchanged (2% deadzone to prevent micro-fluctuation spam)
    ...
    void ApplyToGroup(LODGroup group, LOD[] lods, float[] thresholds)
    {
        ...
        // apply predicted[i] → lods[i].screenRelativeTransitionHeight
        // groups with fewer than 4 LOD levels: apply as many as the group has
        ...
        // final monotonic guard — if any threshold violates decreasing order,
        // force it down. Silent, no LogWarning in hot path.
        for (int i = 1; i < lods.Length; i++)
        {
            if (lods[i].screenRelativeTransitionHeight >= lods[i - 1].screenRelativeTransitionHeight)
            {
                lods[i].screenRelativeTransitionHeight =
                    Mathf.Max(0f, lods[i - 1].screenRelativeTransitionHeight - minThresholdGap);
            }
        }
    ...
    // enforces monotonically decreasing thresholds: LOD0 > LOD1 > LOD2 > LOD3
