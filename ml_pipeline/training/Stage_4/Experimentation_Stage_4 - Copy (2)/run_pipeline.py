...
def main():
    ...
    if skip_optuna:
        print('skip_optuna=True; Skipping tuning. Using hard-coded best parameters.')
        study = type('Study', (), {'best_params': {
            'h1': 256, 'h2': 128, 'h3': 64, 'lr': 1e-3, 'dropout': 0.1
        }})
    else:
    ...
    if epochs_run > 0:
        ...
        if dir_pct < 15.0:
            raise RuntimeError(f"FATAL: Model is learning inverted control (CorrectDir% @ epoch {check_epoch+1}: {dir_pct:.1f}% < 15.0%). Check Reward sign.")
            
        final_pos_sat = history['pos_sat_pct'][-1]
        if final_pos_sat > 35.0:
            raise RuntimeError(f"FATAL: Model has rail-collapsed (pos_sat%: {final_pos_sat:.1f}% > 35.0%). Increase saturation penalty.")
